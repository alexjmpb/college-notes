...
Under this concept, the process of verifying a limit now becomes in a game, where given any positive number $\epsilon$, finding another number namely $\delta$, such that we can make $|f(x) - L| < \epsilon$ by requiring $|x-a| < \delta$, would confirm that $L$ is the limit of $f(x)$ as $x$ approaches $a$. With this, we should have a good intuitive base to talk about the formalization of the definition of a limit.
